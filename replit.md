...
- **Dedicated Product Pages**: All three products have dedicated PDP components lazy-loaded from `ProductDetail.tsx`: `CellunadPDP.tsx` (slug: `cellunad`), `CellubiomePDP.tsx` (slug: `cellubiome`), `CellunovaPDP.tsx` (slug: `cellunova`). Each bypasses the generic `ProductDetailPage` and `ProtocolStackBar`, using product-specific section rhythm, copy, and sticky buy bar. Brand/product rules are in `AGENTS.md`.
- **PDP Desktop Primitives**: Shared desktop layout components in `client/src/components/PdpDesktopPrimitives.tsx` — `PdpHeroShell` (contained 2-panel hero, 28px radius, product-specific split ratio), `PdpSectionRow` (left-led 4/8 grid at lg+), `PdpCenteredSection`, `PdpSupplementFactsShell` (split docs layout), `PdpFaqShell` (2-col FAQ with mobile links fallback), `PdpFinalCtaBand` (compressed CTA), `PdpComparisonShell` (wider comparison matrix). All primitives are desktop-only via lg:/xl: Tailwind classes; mobile layout unchanged.
...
- **Order Flow**: Cart drawer (white, conversion-optimized) → `/checkout` (single-page, light bg, no site nav) → POST /api/checkout/session → Stripe hosted checkout → webhook confirms payment → order updated → `/order-confirmed`. Cart drawer has promo code collapsed behind link, cross-sells below CTA, subscription upgrade prompt. Checkout page has express checkout buttons (Apple Pay / Google Pay) above manual form, single-page email + shipping form with inline validation, sticky bottom CTA, trust cues. No multi-step wizard. See AGENTS.md "Cart and checkout rules" for full UX requirements.
...
- **Pre-rendering with SEO injection**: `server/prerender.ts` injects route-specific SEO blocks into every HTML response. Both `server/vite.ts` (dev) and `server/static.ts` (production) call `injectContent()` using `req.originalUrl` pathname (not `req.path`, which Express 5 wildcard routes set to `/`).
...
- **JSON-LD schemas**: Organization (with contactPoint), WebSite on all pages. Product pages add: Product (with AggregateRating), BreadcrumbList, FAQPage. FAQ and Quality pages add: FAQPage, BreadcrumbList. Shop, Science, Privacy, Terms, Shipping add BreadcrumbList.
- **Robots meta**: All prerendered pages include `max-snippet:-1, max-image-preview:large, max-video-preview:-1` for unlimited AI citation.
- **SEO files**: `sitemap.xml`, `robots.txt`, and `llms.txt` are generated by `scripts/generate-seo-files.mjs` and served from `client/public/`. `robots.txt` explicitly allows 11 AI crawlers (GPTBot, OAI-SearchBot, ClaudeBot, Claude-Web, ChatGPT-User, PerplexityBot, Google-Extended, Applebot, Bytespider, CCBot, anthropic-ai). `llms.txt` follows the standard format with per-URL descriptions.
- **API endpoint**: `GET /api/site-content` returns a rich markdown document with brand summary, product details (ingredients, pricing, dosing), system overview, and differentiators.